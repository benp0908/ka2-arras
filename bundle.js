...
var text7Info = ""
var text8Info = ""
var text9Info = "Send your message by pressing /"
...
var playerNameInfo7 = ""
var playerNameInfo8 = ""
var playerNameInfo9 = "Server"

var playerColourInfo1 = 10
var playerColourInfo2 = 7
var playerColourInfo3 = 10
var playerColourInfo4 = 7
var playerColourInfo5 = 10
var playerColourInfo6 = 7
var playerColourInfo7 = 7
var playerColourInfo8 = 7
var playerColourInfo9 = 7
...
const drawChat = (() => {
   ...
   if (chatMessages.length >= 1) { text1Info = chatMessages[0] }
   if (chatMessages.length >= 2) { text2Info = chatMessages[1] }
   if (chatMessages.length >= 3) { text3Info = chatMessages[2] }
   if (chatMessages.length >= 4) { text4Info = chatMessages[3] }
   if (chatMessages.length >= 5) { text5Info = chatMessages[4] }
   if (chatMessages.length >= 6) { text6Info = chatMessages[5] }
   if (chatMessages.length >= 7) { text7Info = chatMessages[6] }
   if (chatMessages.length >= 8) { text8Info = chatMessages[7] }
   if (chatMessages.length >= 9) { text9Info = chatMessages[8] }

   if (playerSent.length >= 1) { playerNameInfo1 = playerSent[0]}
   if (playerSent.length >= 2) { playerNameInfo2 = playerSent[1] }
   if (playerSent.length >= 3) { playerNameInfo3 = playerSent[2] }
   if (playerSent.length >= 4) { playerNameInfo4 = playerSent[3] }
   if (playerSent.length >= 5) { playerNameInfo5 = playerSent[4] }
   if (playerSent.length >= 6) { playerNameInfo6 = playerSent[5] }
   if (playerSent.length >= 7) { playerNameInfo7 = playerSent[6] }
   if (playerSent.length >= 8) { playerNameInfo8 = playerSent[7] }
   if (playerSent.length >= 9) { playerNameInfo9 = playerSent[8] }

   if (playerColour.length >= 1) { playerColourInfo1 = e(playerColour[0]) }
   if (playerColour.length >= 2) { playerColourInfo2 = e(playerColour[1]) }
   if (playerColour.length >= 3) { playerColourInfo3 = e(playerColour[2]) }
   if (playerColour.length >= 4) { playerColourInfo4 = e(playerColour[3]) }
   if (playerColour.length >= 5) { playerColourInfo5 = e(playerColour[4]) }
   if (playerColour.length >= 6) { playerColourInfo6 = e(playerColour[5]) }
   if (playerColour.length >= 7) { playerColourInfo7 = e(playerColour[6]) }
   if (playerColour.length >= 8) { playerColourInfo8 = e(playerColour[7]) }
   if (playerColour.length >= 9) { playerColourInfo9 = e(playerColour[8]) }
    var chatWidth = 350    
    let ctx = document.createElement("canvas").getContext("2d");
    //ctx.globalAlpha = 0.9;
    //drawBar(x - x, chatWidth, y-(size/2) , 40, 7);
    //ctx.globalAlpha = 1;
    //=========================================== • TEXT INFORMATION • =============================================//
    //text.chatTitle.draw("• Chat (Version 2.0)", x - x, y-(size/2)+15, 30, color.guiwhite, 'left')
    //==================== • DRAW AND CALCULATE MESSAGE OFFSET • ==================//
    text.chat1.draw(text1Info, x - x + ctx.measureText("["+playerNameInfo1+"]:").width*2, y + -30*4+15, 16, l.guiwhite, 'left');
    text.chat2.draw(text2Info, x - x + ctx.measureText("["+playerNameInfo2+"]:").width*2, y + -30*3+15, 16, l.guiwhite, 'left');
    text.chat3.draw(text3Info, x - x + ctx.measureText("["+playerNameInfo3+"]:").width*2, y + -30*2+15, 16, l.guiwhite, 'left');
    text.chat4.draw(text4Info, x - x + ctx.measureText("["+playerNameInfo4+"]:").width*2, y + -30+15, 16, l.guiwhite, 'left');
    text.chat5.draw(text5Info, x - x + ctx.measureText("["+playerNameInfo5+"]:").width*2, y + 0+15, 16, l.guiwhite, 'left');
    text.chat6.draw(text6Info, x - x + ctx.measureText("["+playerNameInfo6+"]:").width*2, y + 30+15, 16, l.guiwhite, 'left');
    text.chat7.draw(text7Info, x - x + ctx.measureText("["+playerNameInfo7+"]:").width*2, y + 30*2+15, 16, l.guiwhite, 'left');
    text.chat8.draw(text8Info, x - x + ctx.measureText("["+playerNameInfo8+"]:").width*2, y + 30*3+15, 16, l.guiwhite, 'left');
    text.chat9.draw(text9Info, x - x + ctx.measureText("["+playerNameInfo9+"]:").width*2, y + 30*4+15, 16, l.guiwhite, 'left');
    //====================== • DRAW PLAYER NAMES WHO SENT THE MESSAGE • ====================//
    text.player1.draw("[" + playerNameInfo1 + "]:", x - x, y + -30*4+15, 16, playerColourInfo1, 'left');
    text.player2.draw("[" + playerNameInfo2 + "]:", x - x, y + -30*3+15, 16, playerColourInfo2, 'left');
    text.player3.draw("[" + playerNameInfo3 + "]:", x - x, y + -30*2+15, 16, playerColourInfo3, 'left');
    text.player4.draw("[" + playerNameInfo4 + "]:", x - x, y + -30+15, 16, playerColourInfo4, 'left');
    text.player5.draw("[" + playerNameInfo5 + "]:", x - x, y + 0+15, 16, playerColourInfo5, 'left');
    text.player6.draw("[" + playerNameInfo6 + "]:", x - x, y + 30+15, 16, playerColourInfo6, 'left');
    text.player7.draw("[" + playerNameInfo7 + "]:", x - x, y + 30*2+15, 16, playerColourInfo7, 'left');
    text.player8.draw("[" + playerNameInfo8 + "]:", x - x, y + 30*3+15, 16, playerColourInfo8, 'left');
    text.player9.draw("[" + playerNameInfo9 + "]:", x - x, y + 30*4+15, 16, playerColourInfo9, 'left');
});
            function za() {
                    ...
                    g.onmessage = function(c) {
                        ...
                        switch (c.shift()) {
                            ...
                            case "e":
                                ...
                                break;
                            case "9": 
                              playerSent.unshift(c[0])
                              chatMessages.unshift(c[1])
                              playerColour.unshift(c[2])
                              //console.log("RECIEVED CHAT INFO: PLAYER: " + c[0] + " PLAYER MESSAGE: " + c[1])
                            break;
                            case "c":
                                z.cx = c[0];
                                z.cy = c[1];
                        ...
                        if (!b.hideGui) {
                            ...
                            b.mobile && L(.8); {
                                ...
                                if (B.graphical.screenshotMode) W[6].draw("Arras.io", h + 200, m - 2, 15, l.guiwhite, "right");
                                    ...
                                    } else W[6].draw("Voizions Arras", h + 200, m - 42 - 2, 15, l.guiwhite, "right");
            ...
            const e = {
                ...
                servers: [{
                    ...
                    type: "TDM",
                    code: "Voizions Arras",
                    at: p.glitch("arras-custom-server"),
                },{
                    id: "b",
                    type: "TDM",
                    code: "Voizions Arras",
